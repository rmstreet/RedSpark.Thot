...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Skills]') AND [c].[name] = N'CreateDate');
IF @var21 IS NOT NULL EXEC(N'ALTER TABLE [Skills] DROP CONSTRAINT [' + @var21 + '];');
ALTER TABLE [Skills] DROP COLUMN [CreateDate];
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Skills]') AND [c].[name] = N'UpdateDate');
IF @var22 IS NOT NULL EXEC(N'ALTER TABLE [Skills] DROP CONSTRAINT [' + @var22 + '];');
ALTER TABLE [Skills] DROP COLUMN [UpdateDate];

GO

ALTER TABLE [Coment] ADD [CreatedById] int NOT NULL DEFAULT 0;

GO

ALTER TABLE [Coment] ADD [FatherComentId] int NULL;

GO

ALTER TABLE [Coment] ADD [LeadId] int NOT NULL DEFAULT 0;
...
ALTER TABLE [Coment] ADD CONSTRAINT [FK_Coment_Person_CreatedById] FOREIGN KEY ([CreatedById]) REFERENCES [Person] ([Id]) ON DELETE CASCADE;

GO

ALTER TABLE [Coment] ADD CONSTRAINT [FK_Coment_Coment_FatherComentId] FOREIGN KEY ([FatherComentId]) REFERENCES [Coment] ([Id]) ON DELETE NO ACTION;

GO

ALTER TABLE [Coment] ADD CONSTRAINT [FK_Coment_Lead_LeadId] FOREIGN KEY ([LeadId]) REFERENCES [Lead] ([Id]) ON DELETE CASCADE;

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20190723232855_Ajuste-tabelas', N'2.2.6-servicing-10079');
